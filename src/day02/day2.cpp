...
pass_info parse(const std::string& s){
    int mn,mx;
    char letter[2];
    char password[64];
    sscanf(s.c_str(), "%d-%d %c: %s", &mn, &mx, letter, password);

    return { mn, mx, letter, password };
}

bool valid_policy1(const std::string& s){
    auto info = parse(s);
    auto count = std::count(info.password.begin(), info.password.end(), info.letter[0]);
    return count >= info.mn && count <= info.mx;
}

bool valid_policy2(const std::string& s){
    auto info = parse(s);
    bool first_contains = info.password[info.mn-1] == info.letter[0];
    bool second_contains = info.password[info.mx-1] == info.letter[0];
    return first_contains ^ second_contains;
}
